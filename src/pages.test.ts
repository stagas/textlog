...
test('notification settings are the only account page that loads their client script', () => {
  ...
  expect(notifications).toContain('name="bots"')
  expect(notifications).toContain('include bots')
...
test('footer links to stats next to the app host', () => {
  ...
  expect(html).toMatch(/<span>[^<]+ <span aria-hidden="true">\/<\/span> <a class="footer-host-link" href="\/stats">stats<\/a> <span aria-hidden="true">·<\/span> <span class="footer-response-time">__TEXTLOG_RESPONSE_TIME__<\/span><\/span>/)
})

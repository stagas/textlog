...
test('Latest still keeps two replies when a conversation has no recent burst', () => {
  const conversation = [
    reply(4, '2026-08-26 23:00:00'),
    reply(3, '2026-08-24 23:00:00'),
    reply(2, '2026-08-22 23:00:00'),
  ]

  expect(recentConversationReplies(conversation).map(post => post.id)).toEqual([4, 3])
})

...
test('consequential account, content, reporting, and admin flows work over HTTP', async () => {
  ...
  const metaCrawler = await request('/latest', { userAgent: 'meta-externalagent/1.1' })
  expect(metaCrawler.status).toBe(200)
  expect(metaCrawler.headers.get('retry-after')).toBeNull()
  ...
  expect(followedPersonFeed).toContain('<span class="activity-side-status">you&#x27;ve seen it all</span>')
  ...
  expect(adminActivity).toContain('action="/for-you/read-all"')

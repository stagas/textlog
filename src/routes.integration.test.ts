...
test('consequential account, content, reporting, and admin flows work over HTTP', async () => {
  ...
  expect(readForYou).toContain('<span class="activity-side-status">you&#x27;ve seen it all</span>')
  ...
  const adminActivity = await (await request('/activity', { cookie: adminCookie })).text()
  ...
  expect(adminActivity).toContain('action="/activity/read-all"')
  ...
  expect(markedActivity.headers.get('location')).toBe('/activity')
  const readAdminActivity = await (await request('/activity', { cookie: adminCookie })).text()
  expect(readAdminActivity).not.toContain('action="/activity/read-all"')
  ...
  const ordinaryActivity = await (await request('/activity', { cookie: aliceCookie })).text()

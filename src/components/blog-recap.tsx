...
const highlights = [
  ...
  ['02', 'Find your people', null,
    <><a href="/search">Full-text search</a>, <a href="/explore">trending tags, profiles, follows, tag communities,
      and helpful popovers</a> make discovery easy.</>],
  ...
  ['06', 'Built to travel', null,
    <><a href="/latest.rss">RSS</a> and <a href="/latest.atom">Atom feeds</a>, <a href="/api/embed-examples">embeds</a>,{' '}
      <a href="/api">a documented API with write access</a>, and <a href="/dump.zip">a public archive</a> make your
      words portable.</>],
...
export function BlogRecap({ user, posts, pageUrl }: { user: User | null; posts: PostView[]; pageUrl: string }) {
  ...
  return (
    <Layout user={user} title="A lot has happened" pageUrl={pageUrl} social={{
      ...
      image: new URL('/og.png?v=2', pageUrl).href, url: pageUrl, type: 'article', imageAlt: name,
    }}>
      <article className="static-page recap-page">
        <p className="eyebrow">since launch</p>
        <h1>A lot has happened.<br />Quietly, of course.</h1>
        <p className="recap-intro">We started with a simple place to write. Since then, {name} has become more
          expressive, more personal, and easier to carry with you—without getting any louder.</p>
        <section className="recap-highlights" aria-labelledby="recap-highlights-title">
          ...
          <ol>{highlights.map(([marker, title, path, copy]) => (
            <li key={marker}><span className="recap-marker" aria-hidden="true">{marker}</span>
              <div><h3>{path ? <a href={path}>{title}</a> : title}</h3><p>{copy}</p></div></li>
          ))}</ol>
        </section>
      ...
      <div className="recap-closing">
        <p>There’s more to find, and plenty more to come. Thanks for making this small corner of the internet feel alive.</p>

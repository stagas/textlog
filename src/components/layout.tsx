  ...
  const accountMenuPopover = user && (
    <div className="account-menu-popover">
      ...
      <form method="post" action="/account/accounts/new">
        <input type="hidden" name="next" value={currentPath} />
        <button className="account-menu-account account-menu-new" type="submit" title="Create new account">+new</button>
      </form>
      <hr className="account-menu-separator" />
      <LogoutForm>
  ...
  return (
    <html lang="en" className={instantScroll ? 'scroll-instant' : undefined}>
      <head>
        ...
        <link rel="stylesheet" href="/styles.css?v=1677" />
        ...
        {onThreadedPage && (
          <>
            ...
            {(onInfiniteFeedPage || onProfilePage) && <script src="/infinite-scroll.js?v=60" defer />}
          </>
        )}

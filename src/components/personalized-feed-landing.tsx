...
export function PersonalizedFeedLanding({ landingUrl, rssUrl, atomUrl, user, created }: { landingUrl: string;
  rssUrl: string;
  atomUrl: string; user?: User | null; created?: boolean }) {
  return (
    <Layout user={user} title="For You feed" feeds={{ title: 'For You', rss: rssUrl, atom: atomUrl }}>
      <CenteredPanel className="magic-link-page" width="medium">
        ...
        <p>{created
          ...
          : 'This private, read-only feed is available in both formats:'}</p>
        <p><a href={rssUrl}>RSS feed</a>{' · '}<a href={atomUrl}>Atom feed</a></p>
        {created && (
          <label className="magic-link-output">
            <span className="feed-self-link"><a href={landingUrl}>Feed</a></span>
            <output className="form-control magic-link-value api-key-output feed-url-output" tabIndex={0}
              aria-label="Feed landing page URL">{landingUrl}</output>
          </label>
        )}

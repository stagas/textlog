...
function SampleContent({ title, eyebrow, danger = false }: { title: string; eyebrow: string; danger?: boolean }) {
  return <>
    ...
    <h2>{title}</h2>
    <p className="panel-gallery-copy">Representative supporting copy for checking measure, spacing, contrast, and wrapping.</p>
...
export function PanelsGallery({ user }: { user?: User | null }) {
  return <Layout user={user} title="panels gallery">
    <div className="panels-gallery">
      ...
      <section className="panel-gallery-section" aria-labelledby="gallery-widths">
        ...
        <Panel width="narrow"><SampleContent eyebrow="narrow · 440px" title="Authentication panel" /></Panel>
        <Panel width="medium"><SampleContent eyebrow="medium · 560px" title="Confirmation panel" /></Panel>
        <Panel width="wide"><SampleContent eyebrow="wide · 620px" title="Welcome panel" /></Panel>
        <Panel width="fluid"><SampleContent eyebrow="fluid" title="Settings or form panel" /></Panel>
      </section>
      <section className="panel-gallery-section" aria-labelledby="gallery-states">
        ...
        <div className="panel-gallery-grid">
          <Panel width="fluid">
            <p className="eyebrow">form</p><h2>Enter your details</h2>
            <form className="panel-gallery-form">
              <label className="form-label">email<input className="form-control" type="email" placeholder="you@example.com" /></label>
              <label className="form-label">message<textarea className="form-control" rows={3} /></label>
              <button className="button" type="button">continue →</button>
            </form>
          </Panel>
          <Panel width="fluid" tone="danger"><SampleContent eyebrow="destructive action" title="Delete this item?" danger /></Panel>
      ...
      <section className="panel-gallery-section" aria-labelledby="gallery-centered">
        ...
        <div className="panel-gallery-shell-preview">
          <CenteredPanel width="narrow"><SampleContent eyebrow="centered shell" title="Log in" /></CenteredPanel>
        </div>
      </section>
    </div>
  </Layout>

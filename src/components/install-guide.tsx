...
export type InstallPlatform = 'ios-safari' | 'ios-chrome' | 'android-chrome' | 'android-firefox' | 'samsung'
  | 'mobile-other'
...
const guides: Record<InstallPlatform,
  { eyebrow: string; heading: string; intro: string; steps: string[]; note?: string }> = {
    'ios-safari': {
      eyebrow: 'iPhone or iPad · Safari',
      heading: 'Add it from Safari’s Share menu',
      intro:
        'Apple calls installation “Add to Home Screen”. It creates an app icon and opens the site without the usual browser controls.',
      steps: [
      ...
      note:
        'If “Add to Home Screen” is missing, scroll to the bottom of the Share sheet, tap “Edit Actions”, and add it to your favourites.',
    },
    'ios-chrome': {
      ...
      note:
        'On older iOS or Chrome versions, this option may only appear in Safari. Choose “Open in Safari”, then follow the Safari steps.',
    },
    'android-chrome': {
    ...
    'mobile-other': {
      ...
      note:
        'If your browser does not offer an install option, open this page in Safari on iPhone or iPad, or Chrome on Android.',
    },
  }
  ...
  return (
    <Layout user={user} title="install">
      <article className="static-page install-guide">
        ...
        <h1>{guide.heading}</h1>
        <p>
          <MenuLabels>{guide.intro}</MenuLabels>
        </p>
        <h2>Step by step</h2>
        <ol>
          {guide.steps.map((step, index) => (
            <li key={index}>
              <span className="install-step-copy">
                <MenuLabels>{step}</MenuLabels>
              </span>
            </li>
          ))}
        </ol>
        {guide.note && (
          <>
            ...
            <p>
              <MenuLabels>{guide.note}</MenuLabels>
            </p>
          </>
        )}
        ...
        <p>
          Launch {name}{' '}
          from its new Home Screen icon. It should open in its own window, and this installation reminder will no longer
          appear.
        </p>
        <p>

...
export function Feed({ user, cursor, title, path = '/for-you', pageUrl, notificationBanner = false }: {
  ...
  const rows = db.query(`SELECT timeline.* FROM (
    ...
    SELECT NULL id,actor.id user_id,NULL body,f.created_at,NULL parent_id,NULL deleted_at,actor.handle,
      ...
      'user-follow:' || printf('%020d',actor.id) || ':' || printf('%020d',target.id) event_key,
    ...
    SELECT NULL id,actor.id user_id,NULL body,hf.created_at,NULL parent_id,NULL deleted_at,actor.handle,
      ...
      'tag-follow:' || printf('%020d',actor.id) || ':' || hf.tag event_key,
    ...
    <Layout user={user} title={title} pageUrl={pageUrl} notificationBanner={notificationBanner}>
      ...
      {timeline.length
        ? timeline.map(row => row.activity_kind === 'post'
          ? <Post key={row.event_key} p={posts.get(row.id)!} user={user} showReplyCount tappable />
          : (

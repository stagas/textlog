  ...
  return (
    <Layout user={user} title="API">
      <article className="static-page api-docs">
        ...
        <p>
          The public API is a small, read-only way to build feeds, profile cards, post embeds, and live widgets. It
...
events.addEventListener('post', event => {
  const post = JSON.parse(event.data)
        ...
        <p>
          ...
          connections per IP. A limited response uses <code>429</code> and includes <code>Retry-After</code>.
        </p>
        <pre><code>{`{

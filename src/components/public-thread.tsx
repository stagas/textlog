...
export function PublicThread(
  { post, replies = [], social, returnPath, topHref, flatHref, treeHref, flat = false, showForm = true, replyTo }: {
    ...
    returnPath?: string; topHref?: string; flatHref?: string; treeHref?: string; flat?: boolean; showForm?: boolean
    replyTo?: PostView
  },
) {
    ...
    <Layout title={postTitle(post.body, post.moderation_category)} social={social}>
      <div className="post-page-thread public-post-page-thread">
        <div className="thread-root">
          <Post p={post} user={null} tappableParent
          ...
          afterReply={(reply, depth) => showForm && !post.thread_locked && reply.id === replyTo?.id
            ? (
              <div className="inline-reply-compose" style={{
                ...
                <ReplyComposer user={null} replyParent={replyTo} replyPageId={post.id} returnPath={returnPath} inline />
              </div>
            )
            : undefined} />
      </div>

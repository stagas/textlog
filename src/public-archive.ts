...
import { createWriteStream, mkdirSync, renameSync, statSync, readFileSync, writeFileSync } from 'node:fs'
...
export function archivePrivacyVersion(database: Database) {
  return JSON.stringify(database.query("SELECT post_id FROM post_hashtags WHERE tag='private' ORDER BY post_id").all())
}

export function archivePrivacyIsCurrent(database: Database, path: string) {
  try { return readFileSync(`${path}.privacy`, 'utf8') === archivePrivacyVersion(database) }
  catch { return false }
}
...
const archiveBuilds = new Map<string, Promise<Awaited<ReturnType<typeof buildPublicArchive>>>>()

export function createPublicArchive(database: Database, path: string, now = new Date(), pageSize = 1000) {
  const existing = archiveBuilds.get(path)
  if (existing) return existing
  const pending = buildPublicArchive(database, path, now, pageSize).finally(() => archiveBuilds.delete(path))
  archiveBuilds.set(path, pending)
  return pending
}

async function buildPublicArchive(database: Database, path: string, now: Date, pageSize: number) {
  ...
  const privacyVersion = archivePrivacyVersion(database)
  ...
  addPages('posts', `SELECT count(*) AS count FROM posts p JOIN users u ON u.id=p.user_id
      WHERE ${privatePostVisible(-1)} AND p.deleted_at IS NULL AND u.deleted_at IS NULL AND u.suspended_at IS NULL`,
  ...
  addPages('translations', `SELECT count(*) AS count FROM posts p JOIN users u ON u.id=p.user_id
      WHERE ${privatePostVisible(-1)} AND p.translation IS NOT NULL AND p.deleted_at IS NULL
  ...
  addPages('execution-outputs', `SELECT count(*) AS count FROM posts p JOIN users u ON u.id=p.user_id
      WHERE ${privatePostVisible(-1)} AND p.execution_output IS NOT NULL AND p.deleted_at IS NULL
        AND u.deleted_at IS NULL AND u.suspended_at IS NULL`,
  ...
  addPages('locations', `SELECT count(*) AS count FROM post_locations pl JOIN posts p ON p.id=pl.post_id
      JOIN users u ON u.id=p.user_id WHERE ${privatePostVisible(-1)} AND p.deleted_at IS NULL
        AND u.deleted_at IS NULL AND u.suspended_at IS NULL`,
  ...
  addPages('hashtags', `SELECT count(*) AS count FROM post_hashtags ph JOIN posts p ON p.id=ph.post_id
      JOIN users u ON u.id=p.user_id WHERE ${privatePostVisible(-1)} AND p.deleted_at IS NULL AND u.deleted_at IS NULL AND u.suspended_at IS NULL`,
  ...
  addPages('mentions', `SELECT count(*) AS count FROM post_mentions pm JOIN posts p ON p.id=pm.post_id
      ...
      WHERE ${privatePostVisible(-1)} AND p.deleted_at IS NULL AND author.deleted_at IS NULL AND author.suspended_at IS NULL
  ...
  writeFileSync(`${path}.privacy`, privacyVersion)

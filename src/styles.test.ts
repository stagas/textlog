...
describe('in-memory stylesheet', () => {
  ...
  test('only lets standalone write forms consume spare page height', async () => {
    ...
    expect(css).toContain(
      'body:not(.write-page):has(#main-content > .write-compose:not(.edit-write-compose), #main-content .feed-compose-header > .write-compose) > header {',
    )
  ...
  test('makes header navigation actions full-height hit targets', async () => {
    ...
    expect(css).toContain('.password-panel > .auth-secondary {\n  text-align: left;\n}')

import { currentPage, htmlFragment, page, paginationRedirect, safeNext } from './shared'
...
export function registerTagsRoutes(app: Hono) {
  ...
  app.get('/tag/:tag', async c => {
    ...
    if (chunk === null || chunk > 0 && tab === 'followers') return c.text('Invalid feed chunk', 400)
    ...
    const view = <TagFeed user={user} tag={tag} displayName={displayName} aliases={aliases} following={following}
        blocked={blocked}
        ...
        returnPath={returnPath} expandedRootId={expandedRootId} chunk={chunk} initialChunks={initialChunks} />

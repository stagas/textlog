...
export function registerIllegalActivityRoutes(app: Hono) {
  app.get('/report-illegal-activity', c => page(<IllegalActivityReport user={currentUser(c.req.raw)}
    values={{ contentUrl: c.req.query('url') || '' }} />))
  ...
  app.post('/report-illegal-activity', async c => {
    ...
    if (limited) return retryPage(page(<IllegalActivityReport user={user} values={values}
      error={authRateLimitMessage(limited.retryAfter)} />, 429), limited.retryAfter)
    const email = (values.email || '').trim().toLowerCase()
    const name = (values.name || '').trim()
    const category = values.category || ''
    let contentUrl: URL | null = null
    try { contentUrl = new URL(values.contentUrl || '') }
    catch {}
    ...
    {
      return page(<IllegalActivityReport user={user} values={values}
        error="Provide the post, category, details, required contact information, and good-faith confirmation." />, 400)
    }
    ...
    if (email) {
      try { await sendReportReceipt(email, reference) }
      catch (error) { console.error('Could not send report receipt', error) }
    }

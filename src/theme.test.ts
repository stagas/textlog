...
import { activeAppearance, activeThemeBackgrounds, appearance, appearanceCookie, appearanceRequestVariant, cornerChoice,
  cornerCookie, fontChoice, fontCookie, fontSizeChoice, fontSizeCookie, primaryFontChoice, primaryFontCookie,
  sansSerifFontChoice, sansSerifFontCookie, themeLogoSvg, themeStyles, versionedAppearance,
  withAppearance } from './theme'
...
test('generated system themes emit the complete static light and dark token contract', async () => {
  ...
  const invariantToken =
    /^(--tap-highlight|--tab-highlight-height|--focus-ring-|--hairline|--gutter|--space-|--font-size-|--form-action-font-size)/
...
test('font preference is validated and emitted by the theme stylesheet', () => {
  ...
  expect(themeStyles(invalid)).toContain('--font-monospace:monospace')

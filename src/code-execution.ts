...
function markerEnd(body: string) {
  let offset = 0
  let fenced = false
...
export function displayedExecutionOutputParts(value: string) {
  ...
  if (lines.length <= MAX_OUTPUT_LINES) return { visible: lines.join('\n'), omitted: '' }
  return {
    visible: [...lines.slice(0, MAX_OUTPUT_LINES - 2), '…', lines.at(-1)!].join('\n'),
    omitted: lines.slice(MAX_OUTPUT_LINES - 2, -1).join('\n'),
  }
}

export function displayedExecutionOutput(value: string) {
  return displayedExecutionOutputParts(value).visible
}

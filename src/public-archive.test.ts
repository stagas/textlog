...
describe('public archive', () => {
  ...
  test('paginates public data and excludes private and unavailable data', async () => {
    ...
    database.run(`CREATE TABLE users(id INTEGER PRIMARY KEY,handle TEXT,email TEXT,bio TEXT,password TEXT,
      ...
      CREATE TABLE posts(id INTEGER PRIMARY KEY,user_id INTEGER,parent_id INTEGER,body TEXT,translation TEXT,
        execution_output TEXT,created_at TEXT,deleted_at TEXT);
    ...
    expect(names).toContain('translations/000001.json')
    expect(names).toContain('execution-outputs/000001.json')
    expect(names).toContain('locations/000001.json')
    ...
    expect(contents).toContain('translated first')
    expect(contents).toContain('visible output')
    expect(contents).toContain('Athens, Greece')
    ...
    expect(contents).not.toContain('private translation')
    expect(contents).not.toContain('private output')

...
import { hiddenReplyGates } from './posts'
...
function apiExtras(database: Database, postIds: number[], viewerId: number) {
  ...
  if (postIds.length && database.query(
    ...
    const rows = database.query(`SELECT l.post_id,l.query,l.latitude,l.longitude,l.display_name,
      m.image_key,m.width,m.height FROM post_locations l JOIN location_map_previews m ON m.cache_key IN ((printf('${LOCATION_ZOOM}:${LOCATION_MAP_STYLE_VERSION}:%.6f:%.6f',l.latitude,l.longitude) || CASE WHEN instr(l.query,'->') > 0 OR instr(l.query,'→') > 0 THEN ':flight:' || replace(l.query,'→','->') ELSE '' END), (printf('${LOCATION_ZOOM}:${LOCATION_MAP_STYLE_VERSION}:%.6f:%.6f',l.latitude,l.longitude) || CASE WHEN instr(l.query,'->') > 0 OR instr(l.query,'→') > 0 THEN ':flight:' || l.query ELSE '' END)) WHERE l.post_id IN
...
export function apiPosts(database: Database, origin: string, options: {
  ...
  return {
    ...
    pagination: { next_cursor: hasMore ? encodeCursor(rows[options.limit - 1].id) : null },
  }
}
...
export function apiReplies(database: Database, origin: string, parentId: number, options: {
  ...
  if (hiddenReplyGates(database, [parentId], options.viewerId ?? -1).hiddenRoots.has(parentId)) {

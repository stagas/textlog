...
app.use('*', async (c, next) => {
  ...
  try {
    ...
    const campaign = url.searchParams.has('reddit') || url.searchParams.has('4chan') || url.searchParams.has('hn')
      || attributedCampaign === 'reddit' || attributedCampaign === '4chan' || attributedCampaign === 'hn'
...
app.use('*', async (c, next) => {
  ...
  const url = new URL(c.req.url)
  const privatePath =
    /^\/(?:enter|forgot-password|reset-password|choose-handle|navigation-check|write|compose|pending-post|pending-follow|pending-poll|activity|admin|search|account|panels-gallery|recap-email(?:-v2)?|interacted-email)(?:\/|$)/
...
app.use('*', async (c, next) => {
  ...
  if (c.req.method !== 'GET' || !wantsHtml
    || ['/choose-handle', '/pending-post', '/pending-follow', '/pending-poll'].includes(c.req.path)) return next()
  const url = new URL(c.req.url)
  const postPage = /^\/post\/[1-9]\d*$/.test(url.pathname)
  const requestedTargetPostId = url.searchParams.get('to')
    || (url.pathname === '/all' ? url.searchParams.get('expand') : null)
  const targetPostId = /^[1-9]\d*$/.test(requestedTargetPostId || '') ? requestedTargetPostId : null
  const returnTo = safeLocalPath(url.pathname + url.search
    + ((postPage || url.pathname === '/all') && targetPostId ? `#post-${targetPostId}` : ''))
  ...
  app.get(path, async () => {
    ...
    return new Response(
      ...
      { headers: {
        'content-type': 'text/javascript; charset=utf-8',
        'cache-control':
          ['/keyboard.js', '/infinite-scroll.js', '/feed-reply.js', '/progressive-pagination.js', '/compose.js', '/contextual-back.js',
              '/reference-follow.js', '/thread-hover-scroll.js', '/thread-expansion.js'].includes(path)

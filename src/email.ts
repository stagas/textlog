...
function button(label: string, url: string) {
  return `<table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" style="margin:20px auto"><tr><td align="center" bgcolor="#749668"><a href="${escapeHtml(url)}" style="display:inline-block;padding:10px 14px;color:#ffffff;font-size:12px;font-weight:700;text-decoration:none;border:1px solid #749668">${escapeHtml(label)} <span aria-hidden="true">→</span></a></td></tr></table>`
}
...
function emailDocument(heading: string, content: string, preheader = heading) {
  ...
  const brand = origin
    ? `<a href="${escapeHtml(origin)}" style="display:inline-block;color:#20231f;text-decoration:none" aria-label="${name}">`
    : '<span style="display:inline-block;color:#20231f">'
  const brandEnd = origin ? '</a>' : '</span>'

  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><title>${escapeHtml(heading)} · ${name}</title></head>
<body style="margin:0;padding:0;background:#ffffff;color:#20231f">
<div style="display:none;max-height:0;overflow:hidden;opacity:0;color:transparent">${escapeHtml(preheader)}&#847; &zwnj;&nbsp;&#847; &zwnj;&nbsp;&#847;</div>
...
<h1 style="margin:0 0 16px;color:#55734a;font-size:20px;line-height:1.35;letter-spacing:-0.5px">${escapeHtml(heading)}</h1>
<div style="color:#3f443d;font-size:13px;line-height:1.65">${content}</div>
</td></tr>
<tr><td align="center" style="padding:14px 0 0;color:#8a9085;font-size:11px;line-height:1.5;text-align:center">Sent by ${name}${origin ? ` · <a href="${escapeHtml(origin)}" style="color:#55734a;text-decoration:none">${escapeHtml(new URL(origin).host)}</a>` : ''}</td></tr>
</table></td></tr></table></body></html>`
}

const paragraph = (content: string) => `<p style="margin:0 0 16px">${content}</p>`
const expiryNotice = (message: string) => notice(`${escapeHtml(message)}<br>If you did not request this, you can safely ignore this email.`)
...
export function sendEmailVerification(email: string, verificationUrl: string, changing = false) {
  ...
  return sendEmail(email, `${action} for ${appName()}`,
    ...
    emailDocument(action, paragraph(changing
      ...
      : 'Confirm this address to finish setting up your account.') + button(action, verificationUrl) +
...
export function sendAccountDeletionConfirmation(email: string, confirmationUrl: string) {
  ...
  return sendEmail(email, `Confirm account deletion · ${name}`,
    ...
    emailDocument('Review account deletion', paragraph(`A request was made to delete your ${escapeHtml(name)} account.`) +
      button('Review account deletion', confirmationUrl) + notice('This link expires in one hour.<br>If this was not you, sign out other sessions to secure your account.')))
}
...
export function sendPasswordEnableConfirmation(email: string, confirmationUrl: string) {
  ...
  return sendEmail(email, `Enable password login · ${name}`,
    ...
    emailDocument('Enable password login', paragraph(`Set a password to add password login to your ${escapeHtml(name)} account.`) +
...
export function sendEmailChangeAuthorization(email: string, confirmationUrl: string) {
  ...
  return sendEmail(email, `Approve email change · ${name}`,
    ...
    emailDocument('Approve email change', paragraph(`A request was made to change the email on your ${escapeHtml(name)} account.`) +
      button('Review email change', confirmationUrl) + notice('This link expires in one hour.<br>If this was not you, sign out other sessions to secure your account.')))
}
...
export function sendMagicLink(email: string, magicUrl: string, code: string, handle?: string) {
  ...
  const name = appName()
  const codeBlock = `<table role="presentation" align="center" cellpadding="0" cellspacing="0" border="0" style="margin:8px auto 20px"><tr><td align="center" style="padding:10px 14px;color:#20231f;background:#ffffff;border:1px solid #d9dbd4;font-size:20px;font-weight:800;letter-spacing:5px;text-align:center">${escapeHtml(code)}</td></tr></table>`
  return sendEmail(email, `${heading} · ${name}`,
    ...
    emailDocument(heading, paragraph(`Use this secure magic link to enter ${escapeHtml(name)}.`) + button(`Enter ${name}`, magicUrl) +
      paragraph('Or enter this six-digit code:') + codeBlock + expiryNotice('The link and code expire in 15 minutes and can only be used once.'),

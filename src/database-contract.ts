...
export type DatabaseDomainOperations = {
  ...
  'auth.resolve': { input: { sessionToken: string | null; bearerToken: string | null; deviceId: string | null;
    now: number }; output: { sessionUser: User | null; apiUser: User | null;
      preferences: { pageSize: PageSizeChoice; density: DensityChoice } } }
  'auth.renewSession': { input: { token: string; now: number }; output: boolean }
  'auth.storeMagicLink': { input: { tokenHash: string; codeHash: string; email: string; userId: number | null;
  ...
  'auth.accountForIdentifier': { input: { identifier: string; isEmail: boolean }; output: {
    id: number; email: string; handle: string; password: string; handleChosenAt: string | null } | null }
  'auth.completePasswordLogin': { input: { userId: number; replacementPasswordHash: string | null;
    userAgent: string; now: number }; output: { session: string } }
  'auth.passwordLoginChallenge': { input: { address: string; now: number; forceCaptcha?: boolean }; output: {
    nonce: string; captcha?: { token: string; image: string } } }
  'auth.validatePasswordLoginForm': { input: { address: string; nonce: string; captchaToken: string;
    captchaAnswer: string; now: number }; output: { status: 'ready' | 'invalid_nonce' | 'invalid_captcha' } }
  'auth.recordFailedPassword': { input: { now: number }; output: boolean }
  'auth.claimInitialHandle': { input: { userId: number; handle: string }; output:
    { status: 'ready' } | { status: 'monthly_limit' } | { status: 'unavailable' } }
  'account.timezone': { input: { userId: number }; output: string | null }
  'account.choices': { input: { userId: number }; output: Array<{
    id: number; handle: string; handle_chosen_at: string | null; primary: boolean; selected: boolean }> }
  'account.select': { input: { userId: number; targetId: number; sessionHash: string }; output:
    { status: 'not_found' } | { status: 'ready'; handleChosen: boolean } }
  ...
  'account.savePushSubscription': { input: { userId: number; endpoint: string; p256dh: string; auth: string;
    deviceId: string; userAgent: string | null; preferencesProvided: boolean; preferences: Record<string, number | null> };
    output: null }
  ...
  'account.storePasswordEnableToken': { input: { userId: number; email: string; tokenHash: string;
    expiresAt: number; now: number }; output: boolean }
  ...
  'account.emailChangeReadiness': { input: { userId: number; email: string }; output:
    { status: 'unavailable' } | { status: 'ready'; passwordHash: string } }
  'account.storeEmailChangeAuthorization': { input: { userId: number; currentEmail: string; newEmail: string;
    tokenHash: string; expiresAt: number; now: number }; output: null }
  'account.emailChangeAuthorization': { input: { tokenHash: string; now: number }; output:
    { userId: number; newEmail: string } | null }
  'account.deleteEmailChangeAuthorization': { input: { userId: number }; output: null }
  'account.emailToken': { input: { tokenHash: string; now: number }; output:
    { userId: number; kind: 'verify' | 'change'; email: string } | null }
  'account.confirmEmailToken': { input: { value: string; now: number }; output:
    { ok: true; kind: 'verify' | 'change' } | { ok: false; reason: 'invalid' | 'email_unavailable' } }
  'account.deletionInfo': { input: { selector: { userId: number } | { tokenHash: string }; now: number }; output:
    { id: number; email: string; passwordHash: string; primary: boolean } | null }
  'account.storeDeletionToken': { input: { userId: number; email: string; tokenHash: string; expiresAt: number;
    now: number }; output: null }
  ...
  'admin.moderateUser': { input: { id: number; actorId: number; action: 'suspend' | 'restore' | 'delete' | 'bot';
    isBot?: boolean; note: string }; output: { status: 'not_found' | 'already_suspended' | 'not_suspended'
      | 'bot_unchanged' } | { status: 'ready'; imageKeys: string[] } }
  'account.securityData': { input: { userId: number; currentSessionHash: string | null; now: number }; output: {
    sessions: SessionView[]; apiKeys: ApiKeyView[]; feedKeys: FeedKeyView[]; passwordEnabled: boolean } }
  'account.issueKey': { input: { kind: 'api' | 'feed'; userId: number; name: string; expiresAt: number | null;
    now: number }; output: { value: string } | null }
  ...
  'account.storeEmailToken': { input: { tokenHash: string; userId: number; kind: 'change'; email: string;
    expiresAt: number }; output: null }
  'account.deleteEmailToken': { input: { tokenHash: string }; output: null }
  'account.saveAppearancePreferences': { input: { userId: number; deviceId: string; pageSize: PageSizeChoice;
    density: DensityChoice; showLinkPreviews: boolean }; output: null }
  ...
  'account.editSettings': { input: { userId: number }; output: {
    isBot: number; botManaged: number; timezone: string; recapEmails: number } | null }
  ...
  'posts.detail': { input: { id: number; viewerId: number }; output:
    { status: 'not_found' } | { status: 'ready'; post: PostView; conversationRootId: number | null } }
  'posts.editData': { input: { id: number; userId: number }; output:
    { status: 'not_found' } | { status: 'forbidden' } | { status: 'ready'; post: PostView; parent: PostView | null } }
  'posts.replyParent': { input: { id: number; userId: number }; output:
    { status: 'not_found' } | { status: 'forbidden' } | { status: 'ready'; post: PostView } }
  'posts.ogData': { input: { id: number }; output: { body: string; handle: string } | null }
  'posts.suggestions': { input: { kind: 'hashtags' | 'mentions'; query: string; viewerId: number }; output:
  ...
  'profiles.blockedPage': { input: { profileId: number; page: number }; output: {
    people: PersonView[]; tags: { tag: string; count: number; viewerFollowing: boolean }[] } }
  'profiles.connectionsPage': { input: { profileId: number; viewerId: number; page: number; tagsPage: number;
    kind: 'following' | 'followers' }; output: { people: PersonView[];
      tags: { tag: string; count: number; viewerFollowing: boolean }[]; total: number } }
  'profiles.postsPage': { input: { profileId: number; viewerId: number; page: number; pageSize: PageSizeChoice;
    kind: 'notes' | 'replies' }; output: PostFeedPage }
  'syndication.load': { input: { kind: 'latest' | 'hot' | 'user' | 'tag' | 'personalized'; origin: string;
    identifier?: string }; output: { status: 'not_found' } | { status: 'redirect'; handle: string }
    | { status: 'ready'; handle?: string; posts: ApiPost[]; viewerHandle?: string;
      activities: Array<{ id: string; title: string; url: string; created_at: string;
        author: { handle: string; url: string } }>; postTitlePrefixes: Record<number, string> } }
  'api.publicRead': { input:
    ...
    | { kind: 'hot'; origin: string; limit: number; cursor: { asOf: string; score: number;
      latestActivityAt: string; createdAt: string; id: number; direction: 'next' | 'previous' } | null }
  ...
  'api.activities': { input: { user: User; origin: string; limit: number;
    cursor: { createdAt: string; key: string } | null; toMe: boolean }; output: unknown }
  ...
  'api.profile': { input: { handle: string; viewerId: number | null; origin: string }; output:
    { status: 'not_found' } | { status: 'redirect'; handle: string }
    | { status: 'ready'; value: unknown; private: boolean } }
  'api.tagDetails': { input: { tag: string; origin: string }; output:
    { status: 'not_found' } | { status: 'ready'; value: unknown } }
  'api.relationships': { input: { kind: 'blocks' | 'following' | 'followers' | 'followingTags' | 'tagFollowers';
    handle?: string; tag?: string; viewerId?: number; origin: string; limit: number; before: number | null }; output:
    { status: 'not_found' } | { status: 'redirect'; handle: string } | { status: 'forbidden' }
    | { status: 'ready'; value: unknown } }
  'api.embedExample': { input: Record<string, never>; output: { postId: number | null; tag: string | null } }
  'api.relationshipMutation': { input: { userId: number; handle: string;
    action: 'follow' | 'unfollow' | 'block' | 'unblock' }; output:
    { status: 'not_found' | 'self' | 'blocked' } | { status: 'ready'; changed: boolean;
      targetId: number; targetHandle: string } }
  'api.createPost': { input: { userId: number; body: string; parentId: number | null; origin: string }; output:
    { status: 'not_found' } | { status: 'rate_limited'; retryAfter: number }
    | { status: 'ready'; id: number; duplicate: boolean; post: ApiPost } }
  'api.updatePost': { input: { userId: number; id: number; body: string; origin: string }; output:
    { status: 'not_found' | 'forbidden' } | { status: 'ready'; post: ApiPost } }
  'api.deletePost': { input: { userId: number; id: number }; output:
    { status: 'not_found' | 'forbidden' } | { status: 'ready'; imageKeys: string[]; parentId: number | null } }
  'api.persistPostPreviews': { input: { postId: number; mode: 'save' | 'replace';
    previews: Array<{ url: string } & LinkPreview> }; output: { obsoleteImageKeys: string[] } }
  ...
  'api.requestSignIn': { input: { email: string; origin: string; now: number }; output:
    { email: string; handle: string; url: string; code: string } | null }
  'api.verifySignIn': { input: { email: string; code: string; userAgent: string; now: number }; output:
    { status: 'invalid' } | { status: 'ready'; token: string; expiresAt: number; user: User } }
  'push.postDelivery': { input: { postId: number; actorId: number }; output: { post: {
    body: string; parentId: number | null; parentHandle: string | null } | null; subscriptions: Array<{
      endpoint: string; p256dh: string; auth: string; userId: number; recipientHandle: string;
      isReply: number; isMention: number; notifyReplies: number; notifyMentions: number }> } }
  'push.followDelivery': { input: { followedId: number }; output: Array<{
    endpoint: string; p256dh: string; auth: string; recipientHandle: string }> }
  ...
  'push.tagFollowDelivery': { input: { actorId: number; tag: string }; output:
    Array<{ endpoint: string; p256dh: string; auth: string }> }
  'push.signupDelivery': { input: { administratorEmails: string[] }; output:
    Array<{ endpoint: string; p256dh: string; auth: string }> }
  ...
  'feeds.recordBanner': { input: { userId: number; userAgent: string | null;
  ...
  'tags.page': { input: { tag: string; viewerId: number; page: number; pageSize: PageSizeChoice;
    tab: 'notes' | 'followers' }; output: TagPageData }
  'embeds.load': { input: { kind: 'latest' | 'hot' } | { kind: 'user'; handle: string }
    | { kind: 'tag'; tag: string } | { kind: 'post'; id: number }; output: EmbedData | null }
  'reports.createIllegalActivity': { input: { postId: number; contentUrl: string; details: string;
    reporterEmail: string | null; reference: string; category: string; reporterName: string | null }; output: boolean }

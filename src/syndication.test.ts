...
function fixture() {
  ...
  database.run(`
    ...
    INSERT INTO posts VALUES
      (1,1,NULL,'hello & <friends> #Root','2026-08-03 10:00:00',NULL),
    ...
    INSERT INTO post_hashtags VALUES(1,'root');
...
describe('RSS and Atom feeds', () => {
  test('serves latest RSS as escaped, public XML', async () => {
    const response = await fixture().request('https://root.mx/latest.rss')
    ...
    expect(body).toContain('<atom:link href="https://root.mx/latest.rss" rel="self"')
    expect(body).toContain('hello &amp; &lt;friends&gt; #Root')
    expect(body).toContain('https://root.mx/post/2')
  ...
  test('serves hot Atom with stable entry IDs and author URLs', async () => {
    const response = await fixture().request('https://root.mx/hot.atom')
    const body = await response.text()

    expect(response.headers.get('content-type')).toBe('application/atom+xml; charset=utf-8')
    expect(body).toContain('<feed xmlns="http://www.w3.org/2005/Atom">')
    expect(body).toContain('<id>https://root.mx/hot.atom</id>')
    expect(body).toContain('<id>https://root.mx/post/1</id>')
    expect(body).toContain('<uri>https://root.mx/u/alice</uri>')
  })

  test('filters user and hashtag feeds and redirects historical handles', async () => {
    const app = fixture()
    const user = await (await app.request('https://root.mx/u/Alice.atom')).text()
    const tag = await (await app.request('https://root.mx/tag/ROOT.rss')).text()
    const alias = await app.request('https://root.mx/u/oldalice.rss')

    expect(user).toContain('hello &amp; &lt;friends&gt; #Root')
    expect(user).not.toContain('a reply')
    expect(tag).toContain('hello &amp; &lt;friends&gt; #Root')
  ...
  test('exposes the same feed formats on API collection URLs', async () => {
    const app = fixture()
    const latest = await app.request('https://root.mx/api/v1/feeds/latest.atom')
    const user = await app.request('https://root.mx/api/v1/users/Alice/posts.rss')
    const tag = await app.request('https://root.mx/api/v1/tags/root/posts.atom')

    expect(latest.status).toBe(200)
    expect(latest.headers.get('content-type')).toBe('application/atom+xml; charset=utf-8')
    expect(await latest.text()).toContain('<id>https://root.mx/api/v1/feeds/latest.atom</id>')
    expect(await user.text()).toContain('https://root.mx/api/v1/users/Alice/posts.rss')
    expect(await tag.text()).toContain('https://root.mx/api/v1/tags/root/posts.atom')
  })

  test('passes ordinary user and hashtag pages through to their HTML routes', async () => {
    const app = fixture()
    expect(await (await app.request('https://root.mx/u/Alice')).text()).toBe('profile Alice')
    expect(await (await app.request('https://root.mx/tag/root')).text()).toBe('tag root')
  })
})

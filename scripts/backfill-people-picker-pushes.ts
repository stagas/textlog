...
import { flushPendingFollowActivityPushes, sendPushForFollow, sendPushForUserFollow, vapidPublicKey } from '../src/push'
...
for (const job of jobs) {
  console.log(
    `people-picker push backfill candidate kind=${job.kind} actor=@${job.actorHandle} target=@${job.targetHandle}`,
  )
}
if (!emit) {
...
for (const job of jobs) {
  ...
  console.log(
    `people-picker push backfill emitted kind=${job.kind} actor=@${job.actorHandle} target=@${job.targetHandle}`,
  )
}

...
const listArgument = (name: string) =>
  Bun.argv.slice(2)
...
function seed(path: string, userCount: number, postCount: number, token: string) {
  ...
  db.transaction(() => {
    for (let index = 0; index < userCount; index++) {
      users.push(
        (addUser.get(`bench_${String(index + 1).padStart(4, '0')}`, `bench-${index + 1}@example.test`,
...
async function request(origin: string, scenario: Scenario, token: string, variant: string): Promise<Sample> {
  ...
  const response = await fetch(url, {
    headers: { cookie, accept: 'text/html', 'user-agent': 'Page benchmark' },
    redirect: 'manual',
  })
...
function summarize(samples: Sample[]) {
  ...
  return {
    ...
    latencyMs: { min: round(timings[0]), mean: round(timings.reduce((a, b) => a + b, 0) / timings.length),
      p50: round(percentile(timings, .5)), p95: round(percentile(timings, .95)), p99: round(percentile(timings, .99)),
...
const pages = [
  ...
  ['@', '/@'],
  ['hot', '/hot'],
  ['profile', `/u/${fixture.handle}`],
  ['post', `/post/${fixture.postId}`],
  ['explore', '/explore'],
] as const
const scenarios: Scenario[] = (['anonymous', 'signed-in'] as const).flatMap(identity =>
  pages
...
try {
  ...
  server = Bun.spawn([process.execPath, '--no-env-file', 'src/server.tsx'], { cwd: join(import.meta.dir, '..'),
    env: { ...process.env, NODE_ENV: 'test', DEV_RELOAD: 'false', ENABLE_MATERIALIZED_MEMORY_CACHE: 'true',
      ...
      LOG_ANONYMOUS: 'false', DISABLE_FEED_WARMING: 'true', IP_PSEUDONYM_SECRET: 'offline-page-benchmark-secret' },
  ...
  for (const scenario of scenarios) {
    ...
    for (let index = 0; index < missSamples; index++) {
      misses.push(
        await request(origin, scenario, token, `benchmark-miss-${scenario.identity}-${scenario.name}-${index}`),
      )
    }
    ...
    if (!jsonOnly) {
      console.log(
        `${scenario.identity.padEnd(9)} ${
          scenario.name.padEnd(8)
        } miss p50 ${result.pageCacheMiss.latencyMs.p50}ms p95 ${result.pageCacheMiss.latencyMs.p95}ms | warm p50 ${result.warm.latencyMs.p50}ms p95 ${result.warm.latencyMs.p95}ms`,
      )
    }
  }
  const report = { generatedAt: new Date().toISOString(), dataset: { users, posts }, samples: {
    pageCacheMiss: missSamples,
    warm: warmSamples,
  },
    note:
      'Page-cache misses use unique harmless query and rendering variants within one process; they retain reusable post fragments and therefore model steady production invalidation, not first-boot cold start.',
    results }

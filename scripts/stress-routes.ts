...
function seedDatabase(path: string, users: number, posts: number, fullParsing: boolean) {
  ...
  database.transaction(() => {
    for (let index = 0; index < users; index++) {
      userIds.push(
        (insertUser.get(`stress_${String(index + 1).padStart(5, '0')}`, `stress_${index + 1}@example.test`,
          'Disposable route benchmark account', 'not-a-login-password') as { id: number }).id,
      )
    }
...
async function benchmark(origin: string, route: string, concurrency: number, durationMs: number): Promise<Result> {
  ...
  return { route, concurrency, durationSeconds: rounded(elapsedSeconds), requests, successfulRequests: successful,
    errors, requestsPerSecond: rounded(requests / elapsedSeconds),
    successfulRequestsPerSecond: rounded(successful / elapsedSeconds),
...
try {
  ...
  const sustainable = Object.fromEntries(routes.map(route => {
    const best = results.filter(result =>
      result.route === route && !result.errors && result.latencyMs.p95 <= p95TargetMs
    )
      .sort((a, b) => b.concurrency - a.concurrency)[0]
    return [route, best
      ? { testedConcurrentClients: best.concurrency, requestsPerSecond: best.successfulRequestsPerSecond,
  ...
  if (!Bun.argv.includes('--json')) {
    ...
    for (const [route, value] of Object.entries(sustainable)) {
      console.log(`  ${route}: ${
        value
          ? `${value.testedConcurrentClients} concurrent clients, ${value.requestsPerSecond} req/s at p95 ${value.p95Ms} ms`
